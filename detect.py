...
class YOLO(object):
    def __init__(self, cfg, cuda=True):
        self.config = cfg
        self.cuda = cuda
        self.class_names = self.config.voc_classes
        self.generate()

    def generate(self):

        self.net = YoloBody(3, self.config.classes).eval()
        ...
        for i in range(3):
            self.yolo_decodes.append(
                DecodeBox(self.config.anchors[i], len(self.class_names), (self.config.img_h, self.config.img_w)))
...
if __name__ == '__main__':
    ...
    cfg.eval_pt = "./output/yolov4.voc.200.pt"
    yolo = YOLO(cfg)
